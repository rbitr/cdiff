...
int main(void)
{
    ...
    double accuracy = (acc_sum + 0.0) / data_rows;

    double loss = log_loss(y1, targets, data_rows);

    printf("printing\n");
    print_array(y1, 100, 1, 'f');

    printf("predictions\n");
    print_array(pred, 100, 1, 'd');
  
    printf("matches\n");
    print_array(acc, 100, 1, 'd');
 
    printf("accuracy sum: %d\n" ,acc_sum); 

    printf("accuracy %f\n", accuracy);

    printf("loss %f\n", loss);

    double * dldy = d_log_loss(y1, targets, data_rows);

    double * dyds = d_sigmoid(y0, data_rows);
